...
6. **TDD 사이클을 준수한다 (MANDATORY).** RED → GREEN → REFACTOR. 테스트 선행 없이 구현을 시작하지 않는다. 커밋 로그에서 테스트가 구현보다 먼저(또는 같은 커밋에) 등장해야 한다. 커버리지 **≥ 70%** (패키지 단위), **도메인 로직은 90%+** 지향.
7. **DDD 레이어 의존성 방향을 준수한다.** 향후 도메인 로직이 추가될 때 다음 방향을 반드시 지킨다:

   ```
   cmd/ (interfaces) → internal/<domain>/application → internal/<domain>/domain
                               ↑
                        internal/<domain>/infrastructure (application Port 구현)
   ```

   - **domain 계층**: `net/http`, `viper`, `zerolog` 등 외부 프레임워크/라이브러리를 import 하지 않는다.
   - **application 계층**: infrastructure 를 직접 import 하지 않는다. Go `interface` 로 선언된 Port 를 사용한다 (accept interfaces, return structs 원칙).
   - **interfaces 계층** (`cmd/`, `internal/server`, `internal/handler`, `internal/middleware`): 라우팅·직렬화·검증만 담당. 비즈니스 로직은 반드시 application 유스케이스를 호출한다.

---

## 점진적 DDD 레이어 전환 계획

프로젝트 현 시점(P0 진행 중) 에는 도메인 로직이 거의 없고 대부분 인프라 층이므로 **루트 수준 재배치를 강제하지 않는다**. 대신 다음 일정에 따라 자연스럽게 도입한다.

| 단계 | DDD 적용 범위 |
|---|---|
| P0-1 ~ P0-5 (완료·진행 중) | 기존 `internal/{config,logger,server}/` 유지. 의존성 방향 원칙만 준수. |
| **P1-6 Worker Pool 부터** | 기능 단위로 3-layer 분리. 최소 형태는 패키지 내 파일 분리: `task.go` (domain) · `pool.go` (application) · `queue_mem.go` (infrastructure). 규모가 커지면 `internal/worker/{domain,application,infrastructure}/` 서브패키지로 승격. |
| P1-7 ~ P1-9 | Connection Pool / Rate Limiter / Circuit Breaker 도 동일 원칙. |
| P3 완료 시점 | 루트 수준 `internal/{domain,application,infrastructure,interfaces}/` 재배치 **여부만** 재평가. 강제 아님. |

**원칙**: "지금 당장 빈 domain 디렉토리를 만들지 않는다. 실제 도메인 객체가 생기는 시점에 함께 만든다."
